...
if (!isset($_POST['name']) || empty($_POST['name']) || !isset($_POST['building']) || empty($_POST['building']) || !isset($_POST['room']) || empty($_POST['room']) || !isset($_POST['time']) || empty($_POST['time']) || !isset($_POST['day']) || empty($_POST['day'])) {
    $error = "Invalid URL.";
} else {
    $name = $_POST['name'];
    ...
    $sql_room = "SELECT room_number, building_code from classrooms
    JOIN buildings
    ON classrooms.buildings_id = buildings.id
    WHERE classrooms.id = " . $room . ";";
    $results_room = $mysqli->query($sql_room);
    if (!$results_room) {
        $mysqli->error;
        exit();
    }
    $row_room = $results_room->fetch_assoc();

    $statement = $mysqli->prepare("INSERT INTO reservations(name, time, day, classrooms_id)
	VALUES(?, ?, ?, ?)");

    $statement->bind_param("sssi", $name, $time, $day, $room);
...
<style>
    ...
    .everything {
        background-image: url("Media/cpa.webp");
    }
            ...
            <div class="container stuff centered">
                ...
                <?php else : ?>
                    <?php if ($is_inserted) : ?>
                        ...
                        <p>Congrats <?php echo $name ?>! You've successfully reserved <?php echo $row_room['building_code'] . " " . $row_room['room_number']; ?> on <?php echo $fullDay; ?> at <span id="res-time"></span>!</p>
